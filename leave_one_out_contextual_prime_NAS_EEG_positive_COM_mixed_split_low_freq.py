...
def train_eval_offline(
  ...
  if skip_training:
    ...
    for step in range(train_steps):
      ...
      if step == 0:
        _ = model.measure_stats(batch, batch_type='valid')
        print("Load Weights from dir : resume_leave_one_out_contextual_low_freq_EEG_mixed_split_positive_COM_30001_steps_1_votes_0.1_cql_alpha_0.05_infeasible_alpha_512_batch_size/ckpt-30000", flush=True)
        model.load_weights(os.path.join("resume_leave_one_out_contextual_low_freq_EEG_mixed_split_positive_COM_30001_steps_1_votes_0.1_cql_alpha_0.05_infeasible_alpha_512_batch_size", "ckpt-30000"))
...
train_eval_offline(
  ...
  eval_freq=100,
  add_summary=True,
  save_dir=f"leave_one_out_contextual_low_freq_EEG_{args.train_steps}_steps_{args.num_votes}_votes_{args.cql_alpha}_cql_alpha_{args.infeasible_alpha}_infeasible_alpha_{args.batch_size}_batch_size",
  loss_type='mse+rank',
  layers=(256, 256, 256),
