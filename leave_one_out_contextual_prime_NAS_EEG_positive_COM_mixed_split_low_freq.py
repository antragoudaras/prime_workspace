...
class HardwareOptProblem:
  ...
  def __init__(self,
    ...
    self.get_training_batch = self.get_all_batch_contextual
    self.get_valid_batch = self.get_all_batch_contextual
  ...
  def get_all_batch(self,):
    ...
    return batch_dict
  def get_all_batch_contextual(self,):
    """Sample i.i.d. from the entire dataset."""
    indices = np.random.randint(1, 
                                self.dataset._top, self._batch_size)
  ...
  def get_full_valid_batch(self,):
    ...
    batch_x, batch_y = self.dataset._get_batch_(indices)
    batch_dict = dict()
    batch_dict['design'] = batch_x
    batch_dict['objective'] = batch_y
    return batch_dict
...
class PRIMEDataset(tf.Module):
  ...
  def get_score_function(self,):
    ...
    print ('Score stats: ')
    print ('--------------------------------------------')
    print ('Max: ', scores.max(), 'with context:', self._tf_dataset['raw_context'][self._tf_dataset['argsort'][-1]])
    print ('Min: ', scores.min(), 'with context:', self._tf_dataset['raw_context'][self._tf_dataset['argsort'][0]])
  ...
  def _convert_to_tf_dataset(self,):
    ...
    for p in parsed_dataset:
      ...
      if p in self._active_training_keys + self._context_keys:
  ...
  def _get_contextual_batch(self, indices):
    """Sample a batch from the dataset."""
    all_train_elements = []  # this is the training elements in one-hot form
    all_test_elements = []  # this is the evaluation fields (area, runtime, score, etc)
    ...
    if not self.contextual:
      ...
      for idx, firefly in enumerate(best_scores):
        ...
        for key in self._active_training_keys + self._context_keys:
          designs.append(self.initial_dataset[key][firefly])
        print('{} Context Score: {}'.format(idx+1, scores[firefly]))
        print('{} Design Paramters & Context: {}'.format(idx+1, designs))
        designs = designs[:-1] #remove the context from the list
        designs = np.expand_dims(np.array(designs), axis=0)
        fireflies.append(designs)
      integer_fireflies = np.array(fireflies).squeeze(axis=1)
...
def train_eval_offline(
  ...
  training_dict['contextual'] = contextual
  if contextual:
  ...
  if skip_training:
    ...
    for step in range(train_steps):
      ...
      if step == 0:
        _ = model.measure_stats(batch, batch_type='valid')
        print("Load Weights from dir : saved_weights_EEG_contextual_leave_one_out/leave_one_out_contextual_low_freq_EEG_mixed_split_positive_COM_30001_steps_1_votes_0.1_cql_alpha_0.05_infeasible_alpha_512_batch_size_30000", flush=True)
        model.load_weights(os.path.join("saved_weights_EEG_contextual_leave_one_out", "leave_one_out_contextual_low_freq_EEG_mixed_split_positive_COM_30001_steps_1_votes_0.1_cql_alpha_0.05_infeasible_alpha_512_batch_size_30000"))
    ...
    if save_dir is not None:
      ...
      model.save_weights(os.path.join("saved_weights_EEG_contextual_leave_one_out", save_dir+"_"+str(step)), overwrite=True)
  ...
  if enable_discrete_optimizer:
    for idx in range(3):
      ...
      random_dataset.to_excel(os.path.join("contextual_leave_one_out_EEG_low_freq_positive_COM_optimized_params_november", "resume_random_dataset_"+str(idx+1)+"_optimized_mixed_split_contextual_leave_one_out_"+str(train_steps-1)+"_steps_low_freq.xlsx"))


df = pd.read_csv(r'final_dataset_EEG_contextual_low_freq.csv',
            index_col=0,
...
# Drop a patient/subject from the final_dataset_EEG_contextual_low_freq
delete_patient = 2.0
df_new = df_actual.drop(df[df['subject_id'] == str(delete_patient)].index)
# df_new = df_new.reset_index()

df_actual_shuffled = df_new.sample(frac=1, random_state=1) #shuffle the valid configs with their accuracy values

print(df_actual_shuffled)

train_len = int(0.8 * (len(df_actual_shuffled) - 1))
df_train = df_actual_shuffled.iloc[: train_len, :]
df_valid = df_actual_shuffled.iloc[train_len:, :]
...
training_data['param_6'] = np.array(int(1e1)*training_data['param_6'], dtype=np.int32)
training_data['param_6'] = np.array(training_data['param_6'], dtype=np.float32)
...
validation_data['param_6'] = np.array(int(1e1)*validation_data['param_6'], dtype=np.int32)
validation_data['param_6'] = np.array(validation_data['param_6'], dtype=np.float32)
...
discrete:subject_id:float64:true:1,3,4,5,6,7,8,9"""
...
train_eval_offline(
  ...
  eval_freq=100,
  add_summary=True,
  save_dir=f"resume_leave_one_out_contextual_low_freq_EEG_mixed_split_positive_COM_{args.train_steps}_steps_{args.num_votes}_votes_{args.cql_alpha}_cql_alpha_{args.infeasible_alpha}_infeasible_alpha_{args.batch_size}_batch_size",
  loss_type='mse+rank',
  layers=(256, 256, 256),
