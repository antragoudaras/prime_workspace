...
class PRIMEDataset(tf.Module):
  ...
  def get_feasible_probs(self, ):
    ...
    # print(f"Inside get_infeasible_probs, feasible_probs: {probs}, infeasible_probs: {infeasible_probs}")
...
def train_eval_offline(
  ...
  if enable_discrete_optimizer:
    ...
    random_dataset = pd.DataFrame({'param_1': random_data[:, 0], 'param_2': random_data[:, 1], 'param_3': random_data[:, 2], 
                        'param_4': random_data[:, 3]})
    param_2_series = random_dataset['param_2'].squeeze()
    ...
    # random_dataset.to_csv(f'./photonics_optimized_negative_results_worst_80_split/random_dataset_worst_80_train_split{batch_size}_size.csv')
    random_dataset.to_excel(f'./photonics_optimized_negative_results_worst_80_split_correct_param_4/random_dataset_worst_80_train_split{batch_size}_size.xlsx')
    ...
    train_dataset = pd.DataFrame({'param_1': train_data[:, 0], 'param_2': train_data[:, 1], 'param_3': train_data[:, 2], 
                        'param_4': train_data[:, 3]})
    param_2_series = train_dataset['param_2'].squeeze()
    ...
    # train_dataset.to_csv(f'./photonics_optimized_negative_results_worst_80_split/train_dataset_worst_80_train_split{batch_size}_size.csv')
    train_dataset.to_excel(f'./photonics_optimized_negative_results_worst_80_split_correct_param_4/train_dataset_worst_80_train_split{batch_size}_size.xlsx')
    ...
    param_2_series = val_dataset['param_2'].squeeze()
    ...
    # val_dataset.to_csv(f'./photonics_optimized_negative_results_worst_80_split/val_dataset_worst_80_train_split{batch_size}_size.csv')
    val_dataset.to_excel(f'./photonics_optimized_negative_results_worst_80_split_correct_param_4/val_dataset_worst_80_train_split{batch_size}_size.xlsx')
...
df_sorted = df_actual_valid.sort_values(by=["sensitivity"])

train_len = int(0.8 * (len(df_sorted) - 1))
df_train_valid = df_sorted.iloc[: train_len, :]
...
df_actual = df_train_valid.append(df_actual_invalid)
df_train = df_actual.sort_values(by=["sensitivity"])
...
training_data['param_2'] = np.array(int(1e2)*training_data['param_2'], dtype=np.int32)
training_data['param_2'] = np.array(training_data['param_2'], dtype=np.float32)
training_data['param_4'] = np.array(int(1e2)*training_data['param_4'], dtype=np.int32)
training_data['param_4'] = np.array(training_data['param_4'], dtype=np.float32) 
# Making all the parameters discrete
...
train_eval_offline(
  ...
  eval_freq=250,
  add_summary=True,
  save_dir="./photonics_saved_weights_negative_worst_split",
  loss_type='mse+rank',
  layers=(256, 256, 256),
