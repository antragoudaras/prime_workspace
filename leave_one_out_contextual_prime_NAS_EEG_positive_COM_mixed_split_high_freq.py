...
def train_eval_offline(
  ...
  if skip_training:
    ...
    for step in range(train_steps):
      ...
      if step == 0:
        _ = model.measure_stats(batch, batch_type='valid')
        print("Load Weights from dir : saved_weights_EEG_high_freq_contextual_leave_one_out/leave_one_out_contextual_high_freq_EEG_20001_steps_1_votes_0.1_cql_alpha_0.05_infeasible_alpha_512_batch_size_20000", flush=True)
        model.load_weights(os.path.join("saved_weights_EEG_high_freq_contextual_leave_one_out", "leave_one_out_contextual_high_freq_EEG_20001_steps_1_votes_0.1_cql_alpha_0.05_infeasible_alpha_512_batch_size_20000"))
    ...
    if save_dir is not None:
      ...
      model.save_weights(os.path.join("saved_weights_EEG_high_freq_contextual_leave_one_out", save_dir+"_"+str(step)), overwrite=True)
  ...
  if enable_discrete_optimizer:
    for idx in range(3):
      ...
      random_dataset.to_excel(os.path.join("contextual_leave_one_out_EEG_high_freq_optimized_params_december", "random_dataset_"+str(idx+1)+"_optimized_contextual_leave_one_out_high_freq_"+str(train_steps-1)+"_steps.xlsx"))
...
train_eval_offline(
  ...
  eval_freq=100,
  add_summary=True,
  save_dir=f"leave_one_out_contextual_high_freq_EEG_{args.train_steps}_steps_{args.num_votes}_votes_{args.cql_alpha}_cql_alpha_{args.infeasible_alpha}_infeasible_alpha_{args.batch_size}_batch_size",
  loss_type='mse+rank',
  layers=(256, 256, 256),
