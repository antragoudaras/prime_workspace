...
def train_eval_offline(
  ...
  if skip_training:
    ...
    if save_dir is not None:
      ...
      model.save_weights(f'./saved_weights_HGD/{save_dir}_{step}', overwrite=True)
  ...
  if enable_discrete_optimizer:
    ...
    random_dataset.to_csv(f'./HGD_positive_COM_optimized_params/random_dataset_optimized_mixed_split.csv')
    random_dataset.to_excel(f'./HGD_positive_COM_optimized_params/random_dataset_optimized_mixed_split.xlsx')
    ...
    train_dataset.to_csv(f'./HGD_positive_COM_optimized_params/train_dataset_optimized_mixed_split.csv')
    train_dataset.to_excel(f'./HGD_positive_COM_optimized_params/train_dataset_optimized_mixed_split.xlsx')
    ...
    val_dataset.to_csv(f'./HGD_positive_COM_optimized_params/val_dataset_optimized_mixed_split.csv')
    val_dataset.to_excel(f'./HGD_positive_COM_optimized_params/val_dataset_optimized_mixed_split.xlsx')
...
df = pd.read_csv(r'./final_dataset_HGD.csv',
            index_col=0,
...
train_eval_offline(
  ...
  eval_freq=250,
  add_summary=True,
  save_dir='./HGD_discrete_positive_COM_mixed_split',
  loss_type='mse+rank',
  layers=(256, 256, 256),
