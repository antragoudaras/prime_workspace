...
Original file is located at
    https://colab.research.google.com/drive/1gbiM0Qyjxm7fLmxdE9vmtKqGW0uqKJ1y
...
def train_eval_offline(
  ...
  if enable_discrete_optimizer:
    ...
    best_firefly = discrete_optimizer.run_inference(num_iters=10, model=model, mode_opt=True)
...
#     sampled_values = np.random.uniform(low=1.0, high=100.0,
#                                          size=dataset_size)
#     data_dict[key] = sampled_values

#   return data_dict

# def create_dummy_data_firefly(dataset_size, config_dict,
#                       remaining_fields=('accuracy',)):
#   """
#   Create dummy data for training PRIME.
...
# firefly_data = create_dummy_data_firefly(2000, dictionary_to_generate_data)

df = pd.read_csv(r'./datasettesseratriantaokto_egg_CNN_dataset.csv',
            index_col=0,
...
training_data = df_actual.to_dict('list')
for key in training_data:
    training_data[f'{key}'] = np.array(training_data[f'{key}'], dtype=np.float64)

training_data['param_6'] = np.array(10*training_data['param_6'], dtype=np.float64)
...
# # raw_dataset = raw_dataset.shuffle(1000132, reshuffle_each_iteration=False)
# parsed_dataset = raw_dataset.map(parse_prime_tfrecords)

# print(parsed_dataset)

# def get_training_dataset(parameter, dataset_size=409600, batch_size = 4096):
#   pars = parsed_dataset.take(dataset_size)
#   pars = pars.batch(batch_size)
#   pars = pars.as_numpy_iterator()
#   res = np.array([])
#   for p in pars:
#     res = np.append(res, p[f'{parameter}'])
#   return res

# def get_validation_dataset(parameter, dataset_size=10, batch_size = 4096, skip_size=409600):
#   pars = parsed_dataset.skip(skip_size).take(dataset_size)
#   pars = pars.batch(batch_size)
#   pars = pars.as_numpy_iterator()
#   res = np.array([])
#   for p in pars:
...
#               'param_10', 'area', 'runtime', 
#               'infeasible']

# training_dataset = {}
# for parameter in parameters:
#   training_dataset[parameter] = get_training_dataset(parameter, dataset_size=409600, batch_size=4096)

# validation_dataset = {}
# for parameter in parameters:
  # validation_dataset[parameter] = get_validation_dataset(parameter, dataset_size=40960, batch_size=4096, skip_size=409600)
