...
def train_eval_offline(
  ...
  if enable_discrete_optimizer:
    ...
    random_dataset.to_excel(f'./october_photonics_optimized_positive_results_best_80_split/random_dataset_{num_votes}_votes.xlsx')
    ...
    train_dataset.to_excel(f'./october_photonics_optimized_positive_results_best_80_split/train_dataset_{num_votes}_votes.xlsx')
    ...
    val_dataset.to_excel(f'./october_photonics_optimized_positive_results_best_80_split/val_dataset_{num_votes}_votes.xlsx')
...
df_total_sorted = df_total.sort_values(by=["sensitivity"])

valid_train_len = int(0.8 * (len(df_total_sorted) - 1)) 
df_train = df_total_sorted.iloc[: valid_train_len, :] #take 80% for train
df_valid = df_total_sorted.iloc[valid_train_len + 1 : , :] # take 20% as test/validation
...
train_eval_offline(
  ...
  eval_freq=500,
  add_summary=True,
  save_dir="positive_best_split_num_votes_one",
  loss_type='mse+rank',
  layers=(256, 256, 256),
